...
namespace HW.Core.CodeGenerator
{
    /// <summary>
    /// 代码生成器。
    /// <remarks>
    ...
    public class CodeGenerator
    {
        ...
        private static void GenerateSingle(Type modelType, bool ifExsitedCovered = false)
        {
            ...
            var modename= modelType.Name;
            var modelTypeName = modelType.Name.Replace("Sys_", "").Replace("G_", "").Replace("R_", "");
            var keyTypeName = modelType.GetProperty("F_Id")?.PropertyType.Name;
            GenerateIRepository(modelTypeName, keyTypeName, modename, ifExsitedCovered);
            GenerateRepository(modelTypeName, keyTypeName, modename, ifExsitedCovered);
            GenerateIService(modelsNamespace, modelTypeName, modename, keyTypeName, ifExsitedCovered);
            GenerateService(modelsNamespace, modelTypeName, modename, keyTypeName, ifExsitedCovered);
        }
        ...
        private static void GenerateIRepository(string modelTypeName, string keyTypeName,string modefullname, bool ifExsitedCovered = false)
        {
            ...
            content = content.Replace("{ModelsNamespace}", _option.ModelsNamespace)
                ...
                .Replace("{ModelName}", modefullname)
        ...
        /// <param name="keyTypeName"></param>
        /// <param name="ifExsitedCovered"></param>
        private static void GenerateRepository(string modelTypeName, string keyTypeName,string modefullname, bool ifExsitedCovered = false)
        {
            var path = AppDomain.CurrentDomain.BaseDirectory;
            path = path.Substring(0, path.IndexOf("\\bin"));
            var parentPath = path.Substring(0, path.LastIndexOf("\\"));
        ...
        /// <param name="keyTypeName"></param>
        /// <param name="ifExsitedCovered"></param>
        private static void GenerateIService(string modelsNamespace, string modelTypeName,string modefullname, string keyTypeName, bool ifExsitedCovered = false)
        {
        ...
        /// <param name="keyTypeName"></param>
        /// <param name="ifExsitedCovered"></param>
        private static void GenerateService(string modelsNamespace, string modelTypeName,string modefullname, string keyTypeName, bool ifExsitedCovered = false)
        {
